...
[`bodybytes.config`](../bodybytes.config) (at the repo root) seeds the target/board selection and board-specific Kconfig options. The `mt76x8` subtarget carries the `emmc` target feature ([`target/linux/ramips/mt76x8/target.mk`](../openwrt/target/linux/ramips/mt76x8/target.mk)), which `select`s the prompt-less `EMMC_SUPPORT` symbol so the base-files package keeps `/lib/upgrade/emmc.sh` — the `emmc_do_upgrade`/`emmc_copy_config` helpers `platform.sh` calls for eMMC sysupgrade (without them, stage2 fails with `emmc_do_upgrade: not found` and nothing is written to the eMMC). This has to be a target feature: `EMMC_SUPPORT` is prompt-less so it can only be reached via `select` (setting `CONFIG_EMMC_SUPPORT=y` in the seed config is silently dropped by `make defconfig`), and base-files is one package per subtarget, so the ~3 KB `emmc.sh` necessarily ships on all mt76x8 images — inert on the non-eMMC boards, whose `platform.sh` never calls those helpers. `CONFIG_SAMBA4_SERVER_AVAHI=y` builds samba4 with avahi client support so smbd registers and deregisters `_smb._tcp` with avahi-daemon dynamically via D-Bus rather than requiring a static service file. `CONFIG_IMAGEOPT=y` and `CONFIG_VERSIONOPT=y` are required to activate the `VERSION_MANUFACTURER`, `VERSION_PRODUCT`, and `VERSION_MANUFACTURER_URL` symbols - without them those symbols live inside an `if VERSIONOPT` block and are silently dropped by Kconfig regardless of their values.
...
| [`openwrt/target/linux/ramips/mt76x8/base-files/etc/init.d/fstrim-boot`](../openwrt/target/linux/ramips/mt76x8/base-files/etc/init.d/fstrim-boot) | Boot-time eMMC/SD TRIM (START=99): backgrounds a delayed `fstrim -a` after each boot. The device has no RTC and is powered on only on demand, so a wall-clock cron job can't reliably fire; running on boot trims proportional to actual use, and the `sleep` lets very short sessions skip it. Board-gated via `case $(board_name)`. |
...
`rootwait=10 panic=3` is what wires the eMMC into the failed-boot watchdog: bounded `rootwait=10` waits at most 10 s for `/dev/mmcblk0p2` (an eMMC/SD enumerates in well under a second, so this only bites on failure — normal boots are unaffected), then the kernel's `Unable to mount root fs` panic reboots after `panic=3` seconds instead of hanging forever. Because the panic happens before userspace, `init.d/bootcount` never resets the counter, so each failed attempt increments the U-Boot bootcount until `bootlimit=3` trips `altbootcmd` into NOR recovery (the SYSCTL MEMO2 register survives the warm reboot). See [Sysupgrade](#2--sysupgrade) and [uboot.md — Boot counter](uboot.md#boot-counter-failed-boot-recovery).
...
**gzip, not LZMA, for the FIT kernel node.** U-Boot decompresses this node on the MT7628's single-issue MIPS32 24KEc core in software, and LZMA decompression there is very slow (single-digit MB/s) — the dominant cost of the U-Boot boot stage. gzip (zlib inflate) decompresses roughly an order of magnitude faster for a slightly larger compressed image that still fits the 32 MB `kernel` partition, and `fit_load_mmc`'s two-pass read only transfers the actual FIT bytes so the extra size costs a fraction of a second. gzip is chosen over lz4 (which is marginally faster still) because U-Boot already has `CONFIG_GZIP=y`, `fit gzip` is a mainline-exercised path, and lz4 would need `CONFIG_LZ4=y` plus a non-standard `Build/lz4`. (The initramfs cpio inside the recovery kernel is separately LZ4-compressed via `CONFIG_TARGET_INITRAMFS_COMPRESSION_LZ4` — decompressed by the kernel, not U-Boot.)
...
- mDNS (avahi): pins the published hostname (`host-name=bodybytes` in `avahi-daemon.conf`) and enables the `avahi-reprobe` init service. Together with `/etc/hotplug.d/iface/99-avahi-mdns`, this keeps `bodybytes.local` resolving to **both** the IPv4 (`192.168.1.1`) and IPv6 ULA (`fd13:37be:ef00::1`) addresses. The fix is needed because avahi (S61) claims the name on IPv4 before the `br-lan` ULA is assigned a few seconds later; the late IPv6 interface churn makes avahi flag a phantom self-conflict and rename itself to `bodybytes-2.local`. `avahi-reprobe` waits for the ULA at boot and restarts avahi once; the hotplug script re-covers any later address change. This is the accepted upstream workaround (the alternative — `use-ipv6=no` — would drop the AAAA record); see [avahi #340](https://github.com/avahi/avahi/issues/340).
...
- Samba (guarded on `/etc/config/samba4`): sets description to `Bodybytes`; gives the `bodybytes` user a Samba password with `smbpasswd` (the Unix user itself is created **independently** of samba, earlier in `90_defaults` — uid/gid **8000**, via the `user_add`/`group_add` helpers from `/lib/functions.sh` since `adduser` isn't in the image — and also owns the data partition). **`/etc/init.d/samba4 start` is called before `smbpasswd`**: `90_defaults` runs from `uci_apply_defaults` *before* the samba4 service (`START=98`) has generated `/etc/samba/smb.conf`, and without that config `smbpasswd` falls back to its compiled-default `tdbsam` backend and writes into tmpfs `/var/lib/samba` — lost on the next reboot, leaving the persistent `/etc/samba/smbpasswd` empty so every login is rejected (and re-triggered on every factory reset / `sysupgrade -n` / overlay recreate, which re-runs `90_defaults`). Starting the service first writes the config (`passdb backend = smbpasswd`), so the user lands in the persistent passwd file. It adds an authenticated read-write share for `/mnt/data` (uci `users=bodybytes` → `valid users`; **not** `valid_users`, which the samba4 init silently ignores; `browseable=yes` so guests can list it but must log in; `create_mask=0666`, `dir_mask=0777`); and injects `host msdfs = no` into `smb.conf.template` so browsing by the `bodybytes.local` mDNS name isn't rejected by smbd's strict DFS parser (`Hostname … is not ours`). No `force user`: the share writes as the mapped `bodybytes` account, and the `data` partition's root dir is owned by uid 8000 (set with `mkfs -E root_owner=8000:8000`, see [flashing.md §5b](flashing.md#5b--first-install-from-nor-recovery)) so `bodybytes` can create files. mDNS/Bonjour advertisement of `_smb._tcp` is handled by smbd itself via the avahi client library (`CONFIG_SAMBA4_SERVER_AVAHI=y`) - smbd registers with avahi-daemon over D-Bus when it starts and deregisters when it stops; no static service file is needed.
...
**`platform_do_upgrade`** sets `CI_KERNPART="kernel"`, `CI_ROOTPART="rootfs"`, `CI_DATAPART="rootfs_data"`, resets the bootcount register with `devmem 0x1000006c 32 0xB0010000`, then calls `emmc_do_upgrade` followed by an explicit **`sync`**. The `sync` is required: `emmc_upgrade_tar` flushes after the rootfs write but **not** after the kernel (which it writes last), so without it the ~MB kernel FIT can sit in the page/eMMC write cache and be lost on reboot — leaving `p1` partially written, so U-Boot reads the FIT header but its bulk read fails (`MMC read … 0 blocks read: ERROR` → `eMMC boot failed`), a corruption that a second sysupgrade then papers over. `0x1000006c` is the physical address of the MT7628 SYSCTL MEMO2 register; `0xB0010000` packs the magic sentinel `0xB001` in bits [31:16] and count `0` in bits [15:0] — the encoding U-Boot's `BOOTCOUNT_GENERIC`/`SINGLEWORD` backend expects. No NOR write occurs; the NOR env partition is fully read-only from Linux. `bootlimit=3` is permanently fixed in the compiled-in env and the NOR env partition.
