...
| [`openwrt/target/linux/ramips/mt76x8/base-files/etc/uci-defaults/90_defaults`](../openwrt/target/linux/ramips/mt76x8/base-files/etc/uci-defaults/90_defaults) | First-boot board defaults: root password (`bodybytes`); hostname; syslog to `/dev/console`; WiFi SSID, country, channel, WPA3-mixed encryption (`sae-mixed`, key `bodybytes`); full LAN setup (br-lan bridge, 192.168.1.1/24, ip6assign=64, ULA prefix); DHCPv6+RA; avahi mDNS hostname pin; travelmate wwan interface + firewall pre-wiring (see the Travelmate bullet under [Board profiles](#board-profiles) below); two-cert TLS PKI for uhttpd and ttyd (with IPv6 enabled); fstab mount for `data` partition at `/mnt/data`; Samba description, `/mnt/data` share, and `bodybytes` system user (guarded on `/etc/config/samba4`); collectd disk/tcpconns/processes enables and RRD path `/srv/collectd/rrd` (guarded on `/etc/config/luci_statistics`) |
...
- WiFi: `ssid=Bodybytes`, `country=US`, `channel=auto`, `encryption=sae-mixed`, `key=bodybytes` (change via LuCI before use). `channel=auto` (ACS) instead of the autodetect-time fixed channel lets hostapd follow whatever channel a travelmate uplink lands on — the single MT7628AN radio can only run AP+STA concurrently on one shared channel.
...
- Travelmate (main profile only): lets bodybytes join a third-party WiFi network as a station while still running its own AP — single MT7628AN radio, so AP and STA share one channel concurrently (see the WiFi bullet above). Two modes, selected purely by whether the service is enabled: **standalone AP** (default — private LAN, no internet) vs. **hybrid AP+STA relay** (enabled + an uplink configured — internet from the uplink is NAT'd to AP clients via the stock `wan` firewall zone, while bodybytes' own LuCI/Samba/dufs/ttyd stay reachable — but only from bodybytes' *own* AP: the `wan` zone's `input='REJECT'` means those services are not reachable from a client on the uplink side, confirmed on hardware — `uhttpd` listens on all interfaces, the firewall is what blocks it). `90_defaults` (guarded on `[ -x … ]`) runs:
  ```sh
  uci -q set travelmate.global.trm_enabled='1'; uci -q commit travelmate
  ...
  ```
  `setup` is the same idempotent codepath the LuCI **Interface Wizard**'s Save button calls — it pre-creates the `trm_wwan`/`trm_wwan6` DHCP/DHCPv6 network interfaces, points `travelmate.global.trm_iface` at `trm_wwan`, and wires both into the `wan` zone's `network` list, so no manual wizard run is needed; the only step left to the user is adding the uplink station (SSID/password, unknowable until on-site) via the Wireless Stations tab, then flipping `trm_enabled` on.

  `trm_enabled` has to be `'1'` for this whole sequence, not merely set afterward: the bottom of upstream's `travelmate-functions.sh` (sourced by *every* `/etc/init.d/travelmate` invocation, before rc.common dispatches to the requested subcommand) unconditionally `exit 1`s if `trm_enabled='0'` — so with it off, `setup`, `scan`, `enable`, `disable`, and even a self-invoked `stop` all silently do nothing. This is also why the LuCI **Scan** button in Wireless Stations does nothing while the `Enabled` toggle is off (same code path), and why `setup()`'s own trailing `"${trm_init}" restart` (which spawns the daemon while `trm_enabled` is briefly `'1'`) has to be followed by an explicit `stop` here: the daemon's own graceful self-stop (triggered when it next notices `trm_enabled='0'`) is broken by the identical mechanism — its self-invoked `stop` call dies at the same sourcing gate instead of actually stopping — so left alone it loops forever retrying (and failing) to stop itself, `travelmate.runtime.json` stays frozen at `"processing"`, and the LuCI page's poll logic (`overview.js`) disables every button, including Save & Restart, for as long as that stale status persists. The explicit `stop` + `rm -f` above stops the daemon while `trm_enabled` is still `'1'` (so it actually works) and clears the stale runtime/pid files, leaving a freshly-booted device with no daemon running and the LuCI page fully usable. Confirmed on hardware: an uplink channel differing from the AP's own causes a real, radio-wide ACS retune — not just the STA vif bouncing — with a measured ~9 s `AP-DISABLED`→`AP-ENABLED` outage that kicks every AP client and can surface as a transient `bodybytes.local` mDNS lookup failure on the client afterward (avahi itself recovers immediately; not a router misconfiguration). Behaviour tuning is left at package defaults (`trm_captive` on; `trm_proactive`/`trm_netcheck` off).
...
- dufs (guarded on `/etc/config/dufs`, main profile only): sets `enabled='1'` - unlike travelmate/samba4, dufs ships **on** by default, since access is scoped by its own `auth` rules rather than by which directory is served. `serve_path='/mnt/data'` (the partition root - dufs refuses to start if `serve_path` itself doesn't exist); `auth` gets two rules: `bodybytes:bodybytes@/protected:rw` and `@/public:rw` (anonymous, full read-write). Once any `auth` rule exists, everything *not* explicitly matched denies by default, so nothing else on the partition leaks. `allow_upload`/`allow_delete`/`allow_search` are also enabled globally, since dufs ANDs those with the per-path grant - without `allow_upload`/`allow_delete`, `:rw` on either path would be inert. `public`/`protected` themselves are pre-created once (not by this script, and not on every boot) by [`init.d/dufs-dirs`](../openwrt/target/linux/ramips/mt76x8/base-files/etc/init.d/dufs-dirs) below. Reuses the uhttpd cert/key for `tls_cert`/`tls_key`, same reasoning as ttyd above - the CA the user already trusts once covers dufs too.
